...
export function FlightGyroDisplay({ orientation, motion, compact = false }: { orientation: OrientationSample | null; motion: MotionSample | null; compact?: boolean }) {
  ...
  return (
    <section className={`rounded-lg border border-cockpit-line bg-cockpit-900 ${compact ? "p-2" : "p-4"}`}>
      ...
      <div className={`relative mx-auto aspect-square overflow-hidden rounded-full border-2 border-slate-600 bg-cockpit-950 ${compact ? "max-h-28" : "max-h-52"}`}>
        <div
      ...
      <div className={`${compact ? "mt-1" : "mt-3"} flex items-center justify-between text-xs text-slate-500`}>
